...
lemma calegari_freedman_construction {α : Ordinal.{u}} 
  (_A _B : TopologicalMoiety α)
  (_σ : H α 1) (_hσ : IsConvergentATranslation _A _σ ∧ support _σ ⊆ (_B : Set (X α 1)))
  (τ : H α 1) (_hτ : IsConvergentATranslation _B τ) :
  ∀ (h : ℕ → H α 1), (∀ n, support (h n) ⊆ (_A : Set (X α 1))) →
...
theorem strong_distortion_implies_strong_boundedness {G : Type*} [Group G]
  [_inst : StronglyDistorted G] : StronglyBounded G := by
  sorry
